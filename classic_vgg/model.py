...
def train_model(model, dataloaders, test_dict, criterion, optimizer, scheduler, num_epochs, device, result_file):
            ...
            for phase in phases:
                ...
                for inputs, labels in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if (phase == 'val'):
                            l = labels.cpu().numpy().tolist()
                            p = preds.cpu().numpy().tolist()
                            y_true = y_true + l
                            y_pred = y_pred + p

                        if (phase == 'test'):
                            test_csv.writerow([test_dict['patientID'][index],test_dict['fID'][index],str(int(p[0])+1)])
                            index += 1
                ...
                epoch_acc_3 = running_3 / len(dataloaders[phase].dataset)
...
def test_model(model, weights, device):
    model.load_state_dict(weights)
    model.eval()

    #image_testset = {x: datasets.ImageFolder(os.path.join('dataset/test', x), data_transforms[x])
    test_transforms = {
    ...
    image_datasets = {x: datasets.ImageFolder(os.path.join('dataset/', x), test_transforms[x]) for x in ['test']}
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=1, shuffle=False, num_workers=4) for x in ['test']}

    images_path = os.listdir('dataset/test')
    for n, image in enumerate(images_path):
        patientID = image.split('_')[0]
        fID = image.split('_')[1][0]
        print(patientID+" "+fID)

        img = Image.open(os.path.join('dataset/test',image))
